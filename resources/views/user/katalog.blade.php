        ...
        <div class="container mx-auto">
            <main class="bg-white shadow-md rounded-lg p-6">
                <div class="flex justify-between items-center mb-4">
                    <h1 class="text-2xl font-bold">Explore New</h1>
                    <input type="text" placeholder="Search or type a command" class="border px-4 py-2 rounded-md w-1/3">
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach($barang as $item)
                    <div class="bg-white p-4 rounded-lg shadow-md">
                        <img src="{{ $item->gambar }}" alt="{{ $item->nama }}" class="w-full h-40 object-cover rounded-md mb-2">
                        <h2 class="text-lg font-semibold">{{ $item->nama }}</h2>
                        <p class="text-gray-600">Rp{{ number_format($item->harga, 0, ',', '.') }}</p>
                        <div class="mt-2 flex justify-between items-center">
                            <button class="bg-blue-500 text-white px-4 py-2 rounded-md">Tambah</button>
                            <span class="text-gray-500 text-sm">Stock: {{ $item->stok }}</span>
                        </div>
                    </div>
                    @endforeach
                </div>
